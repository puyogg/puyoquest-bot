...
import { sendFullArtEmbed, sendRarityEmbed } from '../helper/message-response';
...
const command: Command = {
  ...
  async execute(message: Discord.Message, args: string[]): Promise<void> {
    ...
    if (indexData && indexData.id && indexData.name && indexData.linkName && indexData.rarestid) {
      if (rarity) {
        sendFullArtEmbed(message, indexData.name, indexData.linkName, rarity);
      } else {
      ...
      if (rarity) {
        sendFullArtEmbed(message, name, name.replace(/\s/g, '_'), rarity);
      } else {
