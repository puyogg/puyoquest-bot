...
import { sendRarityEmbed, sendLoreEmbed } from '../helper/message-response';
...
const command: Command = {
  ...
  async execute(message: Discord.Message, args: string[]): Promise<void> {
    ...
    if (
      ...
      if (rarity) {
        sendLoreEmbed(message, indexData.id, indexData.name, indexData.linkName, rarity);
      } else {
      ...
      if (rarity) {
        sendLoreEmbed(message, charID, name, name.replace(/\s/g, '_'), rarity);
      } else {
