...
export default {
  ...
  async execute(message: Discord.Message, args: string[]): Promise<void> {
    ...
    if (args.length === 0) {
      message.channel.send(`Error: You didn't specify a message to pin.`);
      return;
    }
    ...
    try {
      ...
      targetMsg
        .pin()
        .then(() => message.channel.send('Successfully pinned your requested message.'))
        .catch(() => message.channel.send('Error: There was a problem trying to pin your message.'));
    } catch {
      message.channel.send(`Error: There was a problem finding the message you want to pin.`);
      return;
    }
  },
} as Command;
